...
export class IdeaService {
  ...
  private async vote(idea: IdeaEntity, user: UserEntity, vote: Votes) {
    ...
    } else if (!idea[vote].find(voter => voter.id === user.id)) {
      idea[vote].push(user);
      await this.ideaRepository.save(idea);
    } else {
  ...
  async read(id: string): Promise<IdeaRO> {
    const idea = await this.ideaRepository.findOne({ where: { id }, relations: ['author', 'downvotes', 'upvotes'] });
    if (!idea) {
  ...
  async upvote(id: string, userId: string) {
    let idea = await this.ideaRepository.findOne({ where: { id }, relations: ['upvotes', 'downvotes'] });
    const user = await this.userRepository.findOne({ where: { id: userId } });
